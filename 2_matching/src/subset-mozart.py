...
from mozart.oco2 import subset_for_oco2
from mozart.tccon import subset_for_tccon, read_tccon_observations
from mozart.utils import get_runs, get_run_attributes

logging.basicConfig(format="[%(levelname)s] [%(asctime)s] %(message)s", level=logging.INFO)

parser = argparse.ArgumentParser(
    description="""Subset OCO-2 and TCCON observations to a full set of runs of MOZART."""
)
...
parser.add_argument(
    "--runs-directory",
    type=str,
    help=("Base directory of runs (typically 1_transport/intermediates/MOZART/runs)"),
    required=True,
)
parser.add_argument(
