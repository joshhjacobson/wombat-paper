...
def process_history_file(
    ...
    with xarray.open_dataset(filename)[
        ...
        assert is_same_grid(MOZART_GRID["latitude"], ds["model_latitude"].values)
        assert is_same_grid(MOZART_GRID["longitude"], ds["model_longitude"].values)
        ...
        lon_indices = xarray.DataArray(
            match_to_grid(MOZART_GRID["longitude"], observation_lons),
            dims="sounding_id",
        )
        observation_lats = observation_ds["latitude"][start_index:end_index].values
        lat_indices = xarray.DataArray(
            match_to_grid(MOZART_GRID["latitude"], observation_lats),
            dims="sounding_id",
        )
...
class MOZARTSubsetter(object):
    ...
    def match_observations(self):
        ...
        output_ds = self._match_history_files(
            ...
            "MOZART.SpeciesConc.*_0000z.nc4"
            if self.is_subset_hourly
            else "MOZART.SpeciesConcDaily.*.nc4",
            "SpeciesConc_CO2",
            "vco2",
            "lev",
        ...
        level_edge_ds = self._match_history_files(
            ...
            "MOZART.LevelEdgeDiags.*_0000z.nc4"
            if self.is_subset_hourly
            else "MOZART.LevelEdgeDiagsDaily.*.nc4",
            "Met_PEDGE",
            "pressure_edge",
            "ilev",
...
__all__ = ["MOZARTSubsetter", "match_to_grid"]
