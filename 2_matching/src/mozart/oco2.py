...
from .subsetter import MOZARTSubsetter
...
def subset_for_oco2(
    ...
    observation_ds = (
        xarray.open_dataset(observations_path, decode_times=False).load().sortby("time")
    )

    # HACK(mgnb): hardcoding the unit rather than reading it from the file
    assert observation_ds["time"].attrs["units"] == "seconds since 1970-01-01 00:00:00"
    observation_ds["time"] = (
        "sounding_id",
        (
    ...
    valid_times = np.argwhere(np.logical_not(np.isnat(observation_ds["time"].values)))
    observation_ds = observation_ds.isel(sounding_id=valid_times[:, 0])

    matched_ds = MOZARTSubsetter(
        observation_ds,
        run_directory_species_conc,
        run_directory_level_edge,
        is_subset_hourly,
